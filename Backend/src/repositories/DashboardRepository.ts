...
export class DashboardRepository extends BaseRepository {
  ...
  async obtenerProductosBajoStock(empresa_id: number) {
    return this.query(
  ...
  async otenerTodosLosMovimientos(empresa_id: number) {
  ...
  async obtenerProductosConMasMovimientos(empresa_id: number) {
    return this.query(
      `SELECT 
          p.id, p.nombre,
